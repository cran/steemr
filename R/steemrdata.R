# The functions in this script originated from https://github.com/kharoof/steemRdata, modified by the steemr package.
...
getNodes <- function(){
  ...
  data.table::data.table(node=paste0("https://",nodes$Server), status=nodes$Status)
}
...
getTrending <- function(tag="",limit=100, node){
  ...
  results <- data.table::data.table(author=author, title=title, permlink=permlink, pending_payout=pending_payout, votes=votes, comments=comments, created=created)
  return(results)
}
...
getAccountVotes <- function(user="eroche", node){
  ...
  results <- data.table::data.table(permlink=permlink, percent=percent, date = date, time=time, stringsAsFactors=F)
  return(results)
}
...
getDelegation <- function(user="eroche", node){
  ...
  if(length(vests)>0){
  results <- data.table::data.table(delegator=delegator, delegatee=delegatee, vests = vests, SP=SP, date=date, stringsAsFactors=F)}
  else{
    return(NULL)
  }
...
getTransactions <- function(user,n, node){
  ...
  for( j in 1:res_len) {
      tryCatch({
        data.table::set(data,(i-1)*breaks+j,"timestamp",results[[j]][[2]]$timestamp)
        data.table::set(data,(i-1)*breaks+j,"operation",results[[j]][[2]]$op[[1]])
      }, error=function(e){})

    ## Delegation
      tryCatch({
        data.table::set(data,(i-1)*breaks+j,"delegation.delegator",results[[j]][[2]]$op[[2]]$delegator)
        data.table::set(data,(i-1)*breaks+j,"delegation.delegatee",results[[j]][[2]]$op[[2]]$delegatee)
        data.table::set(data,(i-1)*breaks+j,"vesting_shares",results[[j]][[2]]$op[[2]]$vesting_shares)
      },error=function(e){})

    ## Comments / Posts
    tryCatch({
      data.table::set(data,(i-1)*breaks+j,"author",results[[j]][[2]]$op[[2]]$author)
      data.table::set(data,(i-1)*breaks+j,"parent_author",results[[j]][[2]]$op[[2]]$parent_author)
      data.table::set(data,(i-1)*breaks+j,"permlink",results[[j]][[2]]$op[[2]]$permlink)
      data.table::set(data,(i-1)*breaks+j,"title",results[[j]][[2]]$op[[2]]$title)
      data.table::set(data,(i-1)*breaks+j,"body",results[[j]][[2]]$op[[2]]$body)
    },error=function(e){})
    ...
    tryCatch({
      data.table::set(data,(i-1)*breaks+j,"author",results[[j]][[2]]$op[[2]]$author)
      data.table::set(data,(i-1)*breaks+j,"permlink",results[[j]][[2]]$op[[2]]$permlink)
      data.table::set(data,(i-1)*breaks+j,"weight",results[[j]][[2]]$op[[2]]$weight)
    },error=function(e){})
...
getComments <- function(username, node){
  ...
  pick_out <- results$operation=="comment" &
    results$author==results$username &
    !duplicated(results$permlink) &
    results$title==""
  results <- results[pick_out, c('timestamp', 'permlink', 'parent_author')]
  ...
  results$date <- as.Date(substr(results$timestamp, 1,10))
...
cleanData <- function(discussions){
  ...
  results <- data.table::data.table(author=as.character(rep(NA,n)),
  ...
  for(i in 1 : n) {
    ...
    if(skip==FALSE) {
      if(grepl("tags", results[i,"json"])==TRUE){
        ...
        data.table::set(results,i,"category", tags[1])
        for(j in 1:5){
          if(is.na(tags[j])) {
            data.table::set(results,i,paste0("tag.",j), "NA")
          }
          else {
            data.table::set(results,i,paste0("tag.",j), tags[j])
          }
...
##    results[i,"depth"] <- discussion$depth
    ...
    votes <- data.table::data.table(do.call(rbind, discussion$active_votes))
  ...
  results$total_payout <- results$paid_payout + results$pending_payout

  # results[,datetime:= as.POSIXct(results$created, format="%Y-%m-%dT%H:%M:%S")]
  results$datetime <- as.POSIXct(results$created, format="%Y-%m-%dT%H:%M:%S")
  results$created <- NULL
  results$date <- as.Date(results$datetime)
  results$year_month <- format(results$date, "%Y-%b")


  results$author_rep <- reputation.formatter(results$author_reputation)
  results$author_reputation <- NULL
  results$comments <- results$children
...
cleanAccount <- function(data){
  ...
  data <- data.table::data.table(created=as.POSIXct(data$created, format="%Y-%m-%dT%H:%M:%S"),
